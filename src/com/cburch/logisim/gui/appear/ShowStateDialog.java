...
public class ShowStateDialog extends JDialog implements ActionListener {
	...
	private static boolean badPosition(List<CanvasObject> avoid, CanvasObject shape) {
		for (CanvasObject s : avoid) {
			if (shape.overlaps(s) || s.overlaps(shape))
				return true;
		}
		return false;
	}
	...
	private static DefaultMutableTreeNode findChild(DefaultMutableTreeNode node, InstanceComponent ic) {
	...
	private static class Ref {
		...
		public String toString() {
			...
			if (s != null && s.length() > 0)
				...
				return String.format("%s @ (%d, %d)", ic.getFactory(), loc.getX(), loc.getY());
		}
	}
