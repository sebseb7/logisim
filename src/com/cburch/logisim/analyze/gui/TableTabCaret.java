...
class TableTabCaret {
  private class Listener implements MouseListener, MouseMotionListener,
          KeyListener, FocusListener, TruthTableListener, ActionListener {

    public void cellsChanged(TruthTableEvent event) { }
    public void focusGained(FocusEvent e) { repaint(cursor); }
    public void focusLost(FocusEvent e) { repaint(cursor); }
    ...
    void doKey(char c) {
      ...
      if (newEntry != null && cursor.col < inputs) {
        ...
        boolean updated = model.setVisibleInputEntry(cursor.row,
            cursor.col, newEntry, true);
    ...
    public void structureChanged(TruthTableEvent event) {
      /*
         ...
         int cols = inputs + outputs;
         if (cursor.isValid())
         cursor = pointNear(cursor.row, cursor.col);
         if (markA.isValid())
         markA = pointNear(markA.row, markA.col);
         if (markB.isValid())
         markB = pointNear(markB.row, markB.col);
         if (hover.isValid())
         hover = pointNear(hover.row, hover.col);
         */
      cursor = invalid;
      markA = invalid;
      markB = invalid;
