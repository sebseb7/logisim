...
public class TruthTable {
  ...
  private class Row implements Iterable<Integer> {
    ...
    public int baseIndex() {
      ...
      for (int i = 0; i < inputs.length; i++)
        idx = (idx << 1) | (inputs[i] == Entry.ONE ? 1 : 0);
      return idx;
    }
    ...
    public int duplicity() {
      int count = 1;
      for (int i = 0; i < inputs.length; i++)
        count *= (inputs[i] == Entry.DONT_CARE ? 2 : 1);
      return count;
    }
    ...
    public boolean contains(Row other) {
      return contains(other.baseIndex())
          && (other.dcMask() & ~dcMask()) == 0;
    }
  ...
  private void initColumns() {
    ...
    for (int i = 0; i < outputs; i++)
      columns.add(null /* created lazily */);
  }
  ...
  private boolean setDontCare(Row r, int dc, boolean force, boolean changed[]) {
    ...
    if (!force) {
      for (Integer idx : rNew) {
        if (!identicalOutputs(base, idx))
          return false;
      }
    }
    for (int i = 0; i < rows.size(); i++) {
      Row s = rows.get(i);
      if (!rNew.intersects(s))
        continue;
      if (rNew.contains(s)) {
        for (Integer idx : s)
          mergeOutputs(base, idx, changed);
        rows.remove(i);
      } else {
  ...
  public boolean setVisibleInputEntry(int row, int col, Entry value, boolean force) {
    ...
    if (value == Entry.DONT_CARE) {
      ...
      if (!setDontCare(r, dc, force, changed))
        return false;
      fireRowsChanged();
      for (int ocol = 0; ocol < columns.size(); ocol++) {
        if (changed[ocol])
          fireCellsChanged(ocol);
      }
      return true;
  ...
  private class MyListener implements VariableListListener {
    ...
    private void outputsChanged(VariableListEvent event) {
      ...
      if (action == VariableListEvent.ALL_REPLACED) {
        ...
        for (int b = v.width - 1; b >= 0; b--)
          columns.add(bitIndex - b, null /* lazily created */);
        ...
        if (lost > 0) {
          ...
          while (lost++ != 0)
            columns.add(pos, null /* lazily created */);
        }
      }
    }
    ...
    private void moveInput(int oldIndex, int newIndex) {
      ...
      for (Row r : rows) {
        ...
        int dc0;
        if (moveLeft) {
          idx0 = (i & sameMask) | ((i & moveMask) << moveDist)
              | ((i & blockMask) >> 1);
          dc0 = (dc & sameMask) | ((dc & moveMask) << moveDist)
              | ((dc & blockMask) >> 1);
        } else {
          idx0 = (i & sameMask) | ((i & moveMask) >> moveDist)
              | ((i & blockMask) << 1);
    ...
    private Entry[] inputsChangedForOutput(Entry[] column, VariableListEvent event) {
      ...
      if (action == VariableListEvent.ADD) {
        ...
        if (delta > 0) {
          for (int b = 0; b < v.width; b++)
            column = moveInputForOutput(column,
                newIndex - delta - b,
                newIndex - b);
        } else if (delta < 0) {
          for (int b = v.width - 1; b >= 0; b--)
  ...
  public String getInputHeader(int col) { return model.getInputs().bits.get(col); }
  public String getOutputHeader(int col) { return model.getOutputs().bits.get(col); }
  public int getInputIndex(String input) { return model.getInputs().bits.indexOf(input); }
  public int getOutputIndex(String output) { return model.getOutputs().bits.indexOf(output); }
  public List<Var> getInputVariables() { return model.getInputs().vars; }
  public List<Var> getOutputVariables() { return model.getOutputs().vars; }
  public Var getInputVariable(int index) { return model.getInputs().vars.get(index); }
  public Var getOutputVariable(int index) { return model.getOutputs().vars.get(index); }
  public int getInputColumnCount() { return model.getInputs().bits.size(); }
  public int getOutputColumnCount() { return model.getOutputs().bits.size(); }
  public int getRowCount() { return 1 << model.getInputs().bits.size(); }
  public int getVisibleRowCount() { return rows.size(); }
