...
public class Parser {
      ...
      if (t.type == TOKEN_IDENT || t.type == TOKEN_CONST) {
        ...
        if (current != null) {
          push(stack,
              ...
              new Token(TOKEN_AND, t.offset,
                S.get("implicitAndOperator"), Expression.Notation.IMPLICIT_AND_PRECEDENCE));
        }
        ...
        if (current != null) {
          push(stack,
              current,
              Expression.Notation.IMPLICIT_AND_PRECEDENCE,
              new Token(TOKEN_AND, t.offset, 0,
                S.get("implicitAndOperator"), Expression.Notation.IMPLICIT_AND_PRECEDENCE));
        }
        push(stack, null, -2, t);
        current = null;
        ...
        current = popTo(stack, Expression.Notation.IMPLICIT_AND_PRECEDENCE, current);
      } else {
        if (current == null) {
  ...
  private static void push(ArrayList<Context> stack,
      Expression expr, int level, Token cause) {
