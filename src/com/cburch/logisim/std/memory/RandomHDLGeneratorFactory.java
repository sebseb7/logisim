...
public class RandomHDLGeneratorFactory extends AbstractHDLGeneratorFactory {
  ...
  public ArrayList<String> GetModuleFunctionality(Netlist TheNetlist,
    ...
    if (HDLType.equals(Settings.VHDL)) {
      ...
      Contents.add("   BEGIN");
      Contents.add("      IF (GlobalClock'event AND (GlobalClock = '1')) THEN");
      Contents.add("         IF (s_reset = '1') THEN s_output_reg <= s_InitSeed( ("
          + NrOfBitsStr + "-1) DOWNTO 0 );");
      Contents.add("         ELSIF (ClockEnable = '1' AND enable = '1') THEN");
      Contents.add("            s_output_reg <= s_current_seed(("
          + NrOfBitsStr + "+11) DOWNTO 12);");
      Contents.add("         END IF;");
      Contents.add("      END IF;");
      Contents.add("   END PROCESS make_output;");
    } else {
      Contents.add("   assign Q = s_output_reg;");
      Contents.add("   assign s_InitSeed = (" + SeedStr + ") ? "
          + SeedStr + " : 48'h5DEECE66D;");
      ...
      Contents.add("   assign s_start = ((ClockEnable&enable)|((s_reset_reg == 3'b101)&~clear)) ? 1'b1 : 1'b0;");
      Contents.add("   assign s_mult_shift_next = (s_reset) ? 36'd0 :");
      Contents.add("                              (s_start_reg) ? 36'h5DEECE66D : {1'b0,s_mult_shift_reg[35:1]};");
      Contents.add("   assign s_seed_shift_next = (s_reset) ? 48'd0 :");
      Contents.add("                              (s_start_reg) ? s_current_seed : {s_seed_shift_reg[46:0],1'b0};");
      Contents.add("   assign s_mult_busy = (s_mult_shift_reg == 0) ? 1'b0 : 1'b1;");
      Contents.add("   assign s_mac_lo_in_1 = (s_start_reg|s_reset) ? 25'd0 : {1'b0,s_mac_lo_reg[23:0]};");
      Contents.add("   assign s_mac_lo_in_2 = (s_start_reg) ? 25'hB :");
      Contents.add("                          (s_mult_shift_reg[0]) ? {1'b0,s_seed_shift_reg[23:0]} : 25'd0;");
      Contents.add("   assign s_mac_hi_in_2 = (s_start_reg) ? 0 : s_mac_hi_reg;");
      Contents.add("   assign s_mac_hi_1_next = (s_mult_shift_reg[0]) ? s_seed_shift_reg[47:24] : 0;");
      Contents.add("   assign s_busy_pipe_next = (s_reset) ? 2'd0 : {s_busy_pipe_reg[0],s_mult_busy};");
      Contents.add("");
      Contents.add("   always @(posedge GlobalClock)");
      Contents.add("   begin");
      Contents.add("      if (s_reset) s_current_seed <= s_InitSeed;");
      Contents.add("      else if (s_busy_pipe_reg == 2'b10) s_current_seed <= {s_mac_hi_reg,s_mac_lo_reg[23:0]};");
      Contents.add("   end");
      Contents.add("");
      Contents.add("   always @(posedge GlobalClock)");
      Contents.add("   begin");
      ...
      Contents.add("   always @(posedge GlobalClock)");
      Contents.add("   begin");
      Contents.add("      if (s_reset) s_output_reg <= s_InitSeed[("
          + NrOfBitsStr + "-1):0];");
      Contents.add("      else if (ClockEnable&enable) s_output_reg <= s_current_seed[("
          + NrOfBitsStr + "+11):12];");
      Contents.add("   end");
    }
    return Contents;
  ...
  public SortedMap<String, Integer> GetParameterMap(Netlist Nets,
    ...
    int seed = ComponentInfo.GetComponent().getAttributeSet()
        .getValue(Random.ATTR_SEED);
    if (seed == 0)
      seed = (int) System.currentTimeMillis();
    ParameterMap.put(NrOfBitsStr, ComponentInfo.GetComponent()
        .getAttributeSet().getValue(StdAttr.WIDTH).getWidth());
  ...
  public SortedMap<String, String> GetPortMap(Netlist Nets,
    ...
    if (ClockNetName.isEmpty()) {
      GatedClock = true;
      Reporter.AddError("Found a gated clock for component \"Random\" in circuit \""
          + Nets.getCircuitName() + "\"");
      Reporter.AddError("This RNG will not work!");
    }
    if (ComponentInfo.GetComponent().getAttributeSet()
        .containsAttribute(StdAttr.EDGE_TRIGGER)) {
      ActiveLow = ComponentInfo.GetComponent().getAttributeSet()
          .getValue(StdAttr.EDGE_TRIGGER) == StdAttr.TRIG_FALLING;
    }
    ...
    PortMap.putAll(GetNetMap("clear", true, ComponentInfo, Random.RST,
    ...
    if (HDLType.equals(Settings.VHDL)
      ...
      output += "(0)";
    PortMap.putAll(GetNetMap(output, true, ComponentInfo, Random.OUT,
          Reporter, HDLType, Nets));
    return PortMap;
  }
