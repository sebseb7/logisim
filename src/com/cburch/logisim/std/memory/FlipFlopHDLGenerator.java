...
public class FlipFlopHDLGenerator extends HDLGenerator {
  ...
  FlipFlopHDLGenerator(ComponentContext ctx, String name, String displayName,
    ...
    clockPort = new ClockPortInfo("Clock", "Enable", portnum++);
  ...
  protected void generateBehavior(Hdl out) {
    if (out.isVhdl) {
      ...
      out.stmt(vhdlUpdate);
      out.stmt();
      out.stmt("make_memory : PROCESS( Clock , Reset , Preset , Enable , s_next_state )");
      out.stmt("BEGIN");
      out.stmt("   IF (Reset = '1') THEN s_current_state_reg <= '0';");
      out.stmt("   ELSIF (Preset = '1') THEN s_current_state_reg <= '1';");
      if (edgeTriggered())
        out.stmt("   ELSIF (Clock'event AND (Clock = '1')) THEN");
      else
        out.stmt("   ELSIF (Clock = '1') THEN");
      out.stmt("      IF (Enable = '1') THEN");
      ...
      if (edgeTriggered()) {
        ...
        out.stmt("   else if (Enable) s_current_state_reg<= s_next_state;");
        out.stmt("end");
      } else {
        out.stmt("always @(*)");
        out.stmt("begin");
        out.stmt("   if (Reset) s_current_state_reg <= 1'b0;");
        out.stmt("   else if (Preset) s_current_state_reg <= 1'b1;");
        out.stmt("   else if (Enable & (Clock == 1)) s_current_state_reg <= s_next_state;");
        out.stmt("end");
      }
    }
  }
